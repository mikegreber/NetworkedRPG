...
void UNCombatComponent::Initialize()
{
	...
	ANCharacter* NCharacter = Cast<ANCharacter>(OwningCharacter);
	if (NCharacter)
	{
...
void UNCombatComponent::UpdateCombatState(ENCombatType InCombatType, ENStance InStance)
{
	...
	Interface->GetMovementSystemComponent()->SetStance(InStance);
...
void UNCombatComponent::OnWeaponStateChange(ENStance InStance)
{
	// TODO
...
void UNCombatComponent::OnMovementSystemUpdated(ENMovementGait InMovementMode, ENRotationMode InRotationMode, ENStance InStance)
{
...
void UNCombatComponent::ReceivedHit(float HitStrength, float ZRotation)
{
	if (DebugCombatComponent > 1)
	{
		Print(GetWorld(), FString::Printf(TEXT("%s"), *FString(__FUNCTION__)));
	}
	
	ZRotation -= GetOwner()->GetActorRotation().GetNormalized().Yaw;
	
	UAnimMontage* MontageToPlay;

	if (FMath::Abs(ZRotation) > 135)
	{
		if (DebugCombatComponent)
		{
			Print(GetWorld(), FString::Printf(TEXT("%s Flinch Back"), *FString(__FUNCTION__)), EPrintType::Log);
		}
		MontageToPlay = FlinchMontages[ENHitReaction::Back];
	}
	else if (FMath::Abs(ZRotation) < 45)
	{
		if (DebugCombatComponent)
		{
			Print(GetWorld(), FString::Printf(TEXT("%s Flinch Front"), *FString(__FUNCTION__)), EPrintType::Log);
		}
		MontageToPlay = FlinchMontages[ENHitReaction::Front];
	}
	else if (ZRotation < 0)
	{
		if (DebugCombatComponent)
		{
			Print(GetWorld(), FString::Printf(TEXT("%s Flinch Left"), *FString(__FUNCTION__)), EPrintType::Log);
		}
		MontageToPlay = FlinchMontages[ENHitReaction::Left];
	}
	else
	{
		if (DebugCombatComponent)
		{
			Print(GetWorld(), FString::Printf(TEXT("%s Flinch Right"), *FString(__FUNCTION__)), EPrintType::Log);
		}
		MontageToPlay = FlinchMontages[ENHitReaction::Right];
	}
	

	if (DebugCombatComponent && MontageToPlay)
	{
		Print(GetWorld(), FString::Printf(TEXT("%s Should play montage!"), *FString(__FUNCTION__)), EPrintType::Success);
	}

	if (!OwningCharacter->GetMesh()->GetAnimInstance()->Montage_Play(MontageToPlay))
	{
		Print(GetWorld(), FString::Printf(TEXT("%s Failed to play montage!"), *FString(__FUNCTION__)), EPrintType::Error);
	}
}
