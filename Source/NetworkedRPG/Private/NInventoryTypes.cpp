...
bool FNArmourSlot::SetItemData(UNItem* InItemData)
{
    ...
    if (IsValid(MeshComponent))
    {
        ...
        if (EquipmentItem)
        {
            MeshComponent->SetRelativeTransform(EquipmentItem->GetTransform());
...
{
    return WeaponActor ? WeaponActor->GetWeaponMesh() : nullptr;
}
...
// SlotType& SearchSlots(TArray<SlotType>& Slots, bool& bSlotFound, TFunctionRef<bool(const FNEquipmentSlot& Slot)> Predicate, bool bEmptyOnly)
{
    for (auto& Slot : Slots)
    {
        if (Predicate(Slot))
